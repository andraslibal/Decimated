	       ...
	       float syssizey,float xshift,float yshift,int do_net_contour)
{
  ...
  for(layr=(num_layers*num_nodes-1);layr>=0;layr--){
    ...
    for (i=0; i<num_pars; i++) 
    	if (!((cmframes[i].color==2)||(cmframes[i].color==3)))
    {
		 ...
		 int one_vortex,int vortex_num)
{
  ...
  if ((color==2)||(color==3))	{Box2 = x_scale*3/5*10.0; Box = Box2/2;}
  else 				{Box2 = x_scale*8/5*10.0; Box = Box2/2;}
		  ...
		  float syssizex,float syssizey)
{
  ...
  for(i=0;i<num_pins;i++){
    ...
    LPinSizex = pin_sites[i].radiusx*200.0;
    LPinSizey = pin_sites[i].radiusy*200.0;
